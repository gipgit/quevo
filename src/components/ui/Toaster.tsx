...
const Toaster: React.FC<ToasterProps> = ({ toast, onClose }) => {
  ...
  const getToastStyles = () => {
    const baseStyles = "fixed top-4 left-1/2 transform -translate-x-1/2 z-50 max-w-[95vw] lg:max-w-[400px] w-full bg-white rounded-lg shadow-2xl border p-4 transition-all duration-300"
    const visibilityStyles = isVisible ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"
    
    const typeStyles = {
      success: "border-green-200 bg-green-50 shadow-green-100 backdrop-blur-sm",
  ...
  return (
    <>
      {/* Top shadow overlay that blends transparently */}
      <div className="fixed top-0 left-0 right-0 h-12 bg-gradient-to-b from-black/20 to-transparent pointer-events-none z-40" />
