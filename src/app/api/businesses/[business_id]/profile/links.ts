...
export async function PUT(
  ...
  try {
    console.log("[PUT /profile/links]", { params, method: request.method });
    const bodyText = await request.text();
    console.log("[PUT /profile/links] body:", bodyText);
    ...
    if (Array.isArray(body.links)) {
      for (const link of body.links) {
        await prisma.businesslink.upsert({
          where: { business_id_link_type: { business_id: business_id, link_type: link.link_type } },
          update: { link_url: link.link_url, visible: link.visible ?? true },
          create: { business_id: business_id, link_type: link.link_type, link_url: link.link_url, visible: link.visible ?? true },
        })
      }
    }
