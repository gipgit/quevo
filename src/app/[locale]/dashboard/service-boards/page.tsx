                    ...
                    <div className={`rounded-xl shadow-sm border overflow-hidden grid grid-cols-1 lg:grid-cols-[10%_42%_30%_18%] gap-x-4 gap-y-2 p-4 lg:p-6 ${
                            ...
                            {board.serviceboardaction.length > 0 ? (
                              ...
                              {board.serviceboardaction.slice(0, 5).map((action, index) => (
                                <div key={action.action_id} className="flex items-start relative">
                                  {/* Timeline dot */}
                                  <div className={`w-3 h-3 rounded-full ${getStatusColor(action.action_status).replace('text-', 'bg-').replace('bg-gray-100', 'bg-gray-300')} mr-2 mt-2 flex-shrink-0`}></div>
                                  {/* Timeline connector (except for last item) */}
                                  {index < Math.min(4, board.serviceboardaction.length - 1) && (
                                    <div className="absolute left-1.5 top-5 w-0.5 h-6 bg-gray-300"></div>
                                  )}
                                  {/* Action content */}
                                  <div className={`rounded-lg px-3 py-2 min-w-0 border flex-1 ${
                                    theme === 'dark' ? 'bg-gray-700 border-gray-500' : 'bg-gray-50 border-gray-300'
                                  }`}>
                      ...
                      <div className="col-span-1 p-1 flex items-center justify-center">
                        <button
                          ...
                          className="px-4 py-3 w-full lg:w-auto bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors flex items-center justify-center gap-2 font-medium"
                        >
                          {t("openBoard")}
