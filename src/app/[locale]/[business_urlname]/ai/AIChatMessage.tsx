...
import AICallOverviewModal from './AICallOverviewModal';
...
export default function AIChatMessage({ message, themeColors, businessProfileImage, businessName, businessId, onServiceRequestComplete, onQuotationComplete, onAvailabilityComplete, onSuggestionClick }: AIChatMessageProps) {
  ...
  // Check if we should show the message bubble
  const shouldShowMessageBubble = () => {
    ...
    // 1. Has content
    // 2. Is streaming
    // 3. Has token info (AI call overview)
    // 4. Has structured data (services, etc.)
    return (
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} w-full relative z-10`}>
      <div className={`flex ${isUser ? 'flex-col items-end' : 'flex-col items-start'} w-full ${message.data?.type === 'services' ? '' : 'max-w-lg lg:max-w-2xl'} relative z-10`}>
        ...
        {shouldShowMessageBubble() && (
          ...
          >
            ...
            {/* AI Call Overview link for OpenAI responses */}
            {message.data?.tokenInfo && !isUser && (
              <div className="mt-2 text-xs opacity-70">
                <div className="flex flex-wrap gap-2 text-xs items-center">
                  <button
                    onClick={() => setShowAICallOverview(true)}
                  ...
                  {message.data.error && (
                    <span className="text-yellow-600">⚠️ {message.data.error}</span>
                  )}
                </div>
              </div>
            )}
      ...
      {/* AI Call Overview Modal */}
      <AICallOverviewModal
        isOpen={showAICallOverview}
        onClose={() => setShowAICallOverview(false)}
        data={{
          prompt: message.data?.promptInfo ? {
            systemPrompt: message.data.promptInfo.systemPrompt,
            userMessage: message.data.promptInfo.userMessage,
            business: message.data.promptInfo.business,
            servicesCount: message.data.promptInfo.servicesCount
          } : undefined,
          response: message.data?.tokenInfo ? {
            content: message.content,
            inputTokens: message.data.tokenInfo.inputTokens,
            outputTokens: message.data.tokenInfo.outputTokens,
            cost: message.data.tokenInfo.cost,
            inputCost: message.data.tokenInfo.inputCost,
            outputCost: message.data.tokenInfo.outputCost
          } : undefined,
